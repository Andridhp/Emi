...
export default function DocumentsScreen() {
  ...
  const pick = async () => {
    ...
    else Alert.alert('Documento guardado', `Se agregó al expediente de ${profileName}. Antes de usar datos extraídos por IA, tendrás que confirmarlos.`, [{ text: 'Ver documento', onPress: () => router.push({ pathname: '/document/[id]', params: { id } }) }, { text: 'Continuar' }]);
  };
  return <AppShell><ProfileHeader eyebrow="EXPEDIENTE FAMILIAR" />
    ...
    <SectionTitle title="Cómo trabaja la IA" /><View style={styles.aiCard}><Text style={styles.aiTitle}>Extrae → propone → tú confirmas</Text><Text style={styles.aiBody}>La IA organiza fechas, mediciones e indicaciones, pero no confirma resultados ni diagnostica. El dato original siempre permanece vinculado al documento.</Text></View>
  </AppShell>;
}
