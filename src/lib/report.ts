import { BirthRecord, EventKind, FamilyEvent, PostpartumRecord, PrenatalRecord, SourceKind } from '@/types/domain';

export type ReportType = 'routine' | 'illness';
export type ReportSection = 'timeline' | 'care' | 'sleep' | 'symptoms' | 'comfort' | 'treatments' | 'growth' | 'prenatal' | 'birthPostpartum' | 'questions';
export type ReportPeriod = '24h' | '7d' | 'since-consultation';
...
export const reportPeriods: { id: ReportPeriod; title: string; days: number }[] = [
  ...
  { id: 'since-consultation', title: 'Desde la última consulta', days: 21 }
];
...
export function createConsultationReport(events: FamilyEvent[], options: ConsultationReportOptions): ConsultationReportData {
  const period = reportPeriods.find((item) => item.id === options.period) ?? reportPeriods[1];
  const reference = options.referenceDate ?? new Date();
  const start = new Date(reference.getTime() - period.days * 24 * 60 * 60 * 1000);
...
export function assessReportReadiness(report: ConsultationReportData): ReportReadiness {
  ...
  return { readyToExport: Boolean(report.events.length || report.prenatalRecord || report.birthRecord || report.postpartumRecord), completed, total: items.length, items };
}
